...
export default function ProjectBuilderPage() {
  ...
  async function importHtmlNow() {
    ...
    if (!html) {
      setToast({ tone: "danger", title: "Import HTML", message: "Paste some HTML first." });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setToast({ tone: "danger", title: "HTML import failed", message: `(${res.status}) ${text}` });
        return;
      }

      setToast({ tone: "success", title: "HTML imported", message: "Import completed. Loading preview…" });
  ...
  async function importZipNow() {
    ...
    if (!zipFile) {
      setToast({ tone: "danger", title: "Import ZIP", message: "Choose a .zip file first." });
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/projects/${projectId}/import/zip`, { method: "POST", body: form });
      const text = await res.text();

      if (!res.ok) {
        setToast({ tone: "danger", title: "ZIP import failed", message: `(${res.status}) ${text}` });
        return;
      }

      setToast({ tone: "success", title: "ZIP imported", message: "Import completed. Loading preview…" });
  ...
  // Finish: generate (Level 2) — DOES NOT rely on server-to-server publish
  async function finishForMeGenerateOnly() {
    ...
    if (!name || !niche) {
      setToast({ tone: "danger", title: "Finish for me", message: "Please fill Business name and Niche (required)." });
      return false;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setToast({ tone: "danger", title: "Finish failed", message: `(${res.status}) ${text}` });
        return false;
      }

      setToast({ tone: "success", title: "Generated", message: "Site generated. Loading preview…" });
      return true;
    } catch (err: any) {
      setToast({
        tone: "danger",
  ...
  async function runConversionAgent() {
    ...
    try {
      ...
      if (!res.ok) {
        setToast({ tone: "danger", title: "Conversion Agent failed", message: `(${res.status}) ${text}` });
        return false;
      }

      setToast({ tone: "success", title: "Conversion Agent applied", message: "Changes applied. Loading preview…" });
      setModal({ open: false });
      return true;
    } catch (err: any) {
      setToast({
  ...
  async function undoLastChange() {
    ...
    try {
      ...
      if (!res.ok) {
        setToast({ tone: "danger", title: "Undo failed", message: `(${res.status}) ${text}` });
        return;
      }
      ...
      setToast({ tone: "success", title: "Undo", message: `${label}. Loading preview…` });
  ...
  // ✅ ONE-CLICK PIPELINE (BROWSER SESSION SAFE)
  // Generate → Conversion Agent → Publish → Preview → Audit
  async function autoCreateAndPublish() {
    if (!projectId) return;
    ...
    try {
      // 1) Generate (Level 2)
      const genOk = await finishForMeGenerateOnly();
      if (!genOk) return;
      ...
      // 2) Conversion Agent
      const convOk = await runConversionAgent();
      if (!convOk) return;
      ...
      // 3) Publish (client-side = includes auth)
      const pubOk = await publishNow();
      if (!pubOk) return;

      setToast({
        tone: "success",
        title: "All done",
        message: "Generated → Optimised → Published. Your site is live.",
      });
    } finally {
      setBusy(false);
    }
      ...
      <div style={{ minHeight: "100vh", background: "#fafafa", fontFamily: "system-ui" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: 24 }}>
          <div style={{ border: "1px solid #eee", borderRadius: 16, background: "white", padding: 20 }}>
            ...
            <p style={{ marginTop: 10, marginBottom: 0, color: "#444" }}>We couldn’t read the project id from the URL.</p>
            <div style={{ marginTop: 16, display: "flex", gap: 10 }}>
              <button
                onClick={() => router.push("/projects")}
                style={{ padding: "10px 14px", borderRadius: 12, border: "1px solid #ddd", background: "white", cursor: "pointer", fontWeight: 600 }}
              >
                Back to projects
              </button>
            </div>
          ...
          >
            <div style={{ padding: 14, borderBottom: "1px solid #eee", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <div style={{ fontWeight: 900, color: "#111" }}>
                {modal.kind === "finish"
                  ? "Business details (used for Auto Create & Publish)"
              ...
              <button
                onClick={() => closeModal()}
                style={{ padding: "8px 12px", borderRadius: 12, border: "1px solid #ddd", background: "white", cursor: "pointer", fontWeight: 700 }}
              >
                Close
              </button>
            </div>
              ...
              {modal.kind === "finish" ? (
                <>
                  <div style={{ fontSize: 13, color: "#555", lineHeight: 1.4 }}>
                    Fill these once. Then use the big green <b>Auto Create & Publish</b> button.
                  </div>

                  <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <label style={{ display: "grid", gap: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#333" }}>Business name (required)</span>
                      <input value={bizName} onChange={(e) => setBizName(e.target.value)} placeholder="e.g. Book A Ride" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 10 }} />
                    </label>

                    <label style={{ display: "grid", gap: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#333" }}>Niche (required)</span>
                      <input value={bizNiche} onChange={(e) => setBizNiche(e.target.value)} placeholder="e.g. Airport shuttles" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 10 }} />
                    </label>

                    <label style={{ display: "grid", gap: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#333" }}>Location</span>
                      <input value={bizLocation} onChange={(e) => setBizLocation(e.target.value)} placeholder="e.g. Auckland, NZ" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 10 }} />
                    </label>

                    <label style={{ display: "grid", gap: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#333" }}>Tagline</span>
                      <input value={bizTagline} onChange={(e) => setBizTagline(e.target.value)} placeholder="e.g. Launch premium sites fast" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 10 }} />
                    </label>

                    <label style={{ display: "grid", gap: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#333" }}>Phone (optional)</span>
                      <input value={bizPhone} onChange={(e) => setBizPhone(e.target.value)} placeholder="(optional)" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 10 }} />
                    </label>

                    <label style={{ display: "grid", gap: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: "#333" }}>Email (optional)</span>
                      <input value={bizEmail} onChange={(e) => setBizEmail(e.target.value)} placeholder="(optional)" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 10 }} />
                    </label>
                  </div>
                ...
                <>
                  ...
                  <div style={{ marginTop: 12, display: "flex", gap: 10, justifyContent: "flex-end" }}>
                    <button
                      ...
                      style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #ddd", background: busy ? "#f3f4f6" : "#111", color: busy ? "#777" : "white", cursor: busy ? "not-allowed" : "pointer", fontWeight: 800 }}
                    >
                ...
                <>
                  ...
                  <textarea
                    ...
                    style={{ marginTop: 12, width: "100%", height: 220, resize: "vertical", borderRadius: 14, border: "1px solid #ddd", padding: 12, fontFamily: "system-ui", fontSize: 13, lineHeight: 1.5 }}
                  />
                  ...
                  <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-end", gap: 10 }}>
                    <button
                      ...
                      style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #ddd", background: busy ? "#f3f4f6" : "#111", color: busy ? "#777" : "white", cursor: busy ? "not-allowed" : "pointer", fontWeight: 900 }}
                    >
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <span style={{ display: "inline-flex", alignItems: "center", padding: "6px 10px", borderRadius: 999, fontSize: 12, border: "1px solid #e5e5e5", background: "white", color: "#111" }} title="Project ID">
              {projectId}
            </span>
          </div>

          <button onClick={() => router.push("/projects")} style={{ padding: "10px 14px", borderRadius: 12, border: "1px solid #ddd", background: "white", cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap" }}>
            Back
          </button>
        </div>
          ...
          <div style={{ border: "1px solid #eee", borderRadius: 16, background: "white", padding: 16 }}>
            ...
            <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
              <button
                ...
                style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #ddd", background: "white", cursor: busy ? "not-allowed" : "pointer", fontWeight: 900 }}
              >
              ...
              <button disabled={busy} onClick={() => setModal({ open: true, kind: "importHtml" })} style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #ddd", background: "white", cursor: busy ? "not-allowed" : "pointer", fontWeight: 700 }}>
                Import HTML
              </button>

              <button disabled={busy} onClick={() => setModal({ open: true, kind: "importZip" })} style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #ddd", background: "white", cursor: busy ? "not-allowed" : "pointer", fontWeight: 700 }}>
                Import ZIP
              </button>

              <button disabled={busy} onClick={() => loadPreview()} style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #ddd", background: "white", cursor: busy ? "not-allowed" : "pointer", fontWeight: 700 }}>
                Load Preview
              </button>

              <button disabled={busy} onClick={() => runAudit()} style={{ padding: "12px 14px", borderRadius: 14, border: "1px solid #ddd", background: "white", cursor: busy ? "not-allowed" : "pointer", fontWeight: 800 }}>
                Run quality check
              </button>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              >
                {audit.state === "idle" ? (
                  <div style={{ color: "#444" }}>
                    Use <b>Auto Create & Publish</b> for the full pipeline. Or run steps manually.
                  </div>
                ) : audit.state === "checking" ? (
                  ...
                  <div>
                    ...
                    {audit.missing.length === 0 && audit.warnings.length === 0 && audit.notes.length === 0 ? <div style={{ color: "#444" }}>No issues found.</div> : null}
                  </div>
                )}
              </div>
            </div>
        ...
        <div style={{ marginTop: 14, fontSize: 12, color: "#666" }}>
          Automation: the green button runs Generate → Conversion Agent → Publish using your browser session (so auth works).
        </div>
      </div>
    </div>
  );
