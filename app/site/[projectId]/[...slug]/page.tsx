// app/site/[projectId]/[...slug]/page.tsx
...
export default async function PublicProjectSubPage({
  params,
}: {
  ...
  const html =
    (pages && typeof pages[path] === "string" && pages[path].trim()) ||
    (pages && typeof pages["/"] === "string" && pages["/"].trim()) ||
        ...
        >
          <div>
            ...
            <div style={{ fontWeight: 900, fontSize: 18 }}>
              {params.projectId}{" "}
              <span style={{ opacity: 0.7, fontWeight: 700 }}>{path}</span>
            </div>
          </div>

          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <Link
              href={`/site/${params.projectId}`}
              style={{
                padding: "8px 12px",
                borderRadius: 999,
        ...
        {!html ? (
          ...
          >
            ...
            <p style={{ marginTop: 8, opacity: 0.75, lineHeight: 1.6 }}>
              Apply a run to write the KV key: <code>{latestKey(params.projectId)}</code>.
            </p>
          </div>
        ) : (
          <iframe
