...
function generateHtml(opts: { projectId: string; runId: string; prompt: string }) {
  ...
  const subtitle =
    p.includes("business")
      ? "A clean modern business site."
      : p.includes("portfolio")
      ? "A creator portfolio with projects."
      : p.includes("landing")
...
export async function POST(
  ...
  try {
    ...
    // ✅ Save latest (keeps /generated working)
    await kv.set("generated:latest", payload);

    // ✅ Save per-run (powers /generated/<runId>)
    ...
    // ✅ IMPORTANT: Always set latest run pointer
    await kv.set("runs:latest", runId);

    return NextResponse.json({
      ok: true,
      projectId,
      runId,
      saved: ["generated:latest", `generated:run:${runId}`, "runs:latest"],
    });
  } catch (e: any) {
    return NextResponse.json(
